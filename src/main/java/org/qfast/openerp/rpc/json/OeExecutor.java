...
public class OeExecutor implements Serializable {
    ...
    public OeExecutor(String protocol, String host, int port, String database,
        ...
        this.v70 = version.getVersionNumber() == 7
                && version.getSubVersion() == 0;
    }

    public OeExecutor(String host, int port, String database, String username,
            String password) throws OeRpcException {

        this("http", host, port, database, username, password);
    }

    public OeExecutor(String host, String database, String username,
            String password) throws OeRpcException {
    ...
    private JsonObject doLogin() throws OeRpcException {
        ...
        JsonObject response = postRequest(client, reqUrl, getCallWith(params));
    ...
    private JSONObject getJsonContextFromLogin(JsonObject result) {
        if (result != null) {
            ...
            if (!result.containsKey("user_context")
                    && result.containsKey("context")) {
    ...
    public Map<String, Object>[] doSearch(String model, List<Object> domain)
    ...
    public Map<String, Object>[] doSearch(String model, List<Object> domain,
            Integer limit) throws OeRpcException {

        return doSearch(model, domain, null, limit);
    }

    public Map<String, Object>[] doSearch(String model, List<Object> domain,
            String order) throws OeRpcException {

        return doSearch(model, domain, null, null, order);
    }

    public Map<String, Object>[] doSearch(String model, List<Object> domain,
            Integer offset, Integer limit) throws OeRpcException {

        return doSearch(model, domain, offset, limit, null);
    }

    public Map<String, Object>[] doSearch(String model, List<Object> domain,
            Integer offset, Integer limit, String order) throws OeRpcException {

        return doSearchMap(model, domain, offset, limit, order, context);
    }

    public String doSearchStr(String model, List<Object> domain,
    ...
    public JSONArray doSearch(String model, List<Object> domain,
            Integer offset, Integer limit, String order,
            Map<String, Object> context) throws OeRpcException {
        return new JSONArray(doSearchStr(model, domain, offset, limit, order, context));
    }

    public Map<String, Object>[] doSearchMap(String model, List<Object> domain,
            Integer offset, Integer limit, String order,
            Map<String, Object> context) throws OeRpcException {
        return OeUtil.convertJsonArrayToMapArray(doSearch(model, domain, offset,
                limit, order, context));
    }
    ...
    public Long doCount(String model, List<Object> domain) throws OeRpcException {
        ...
        argms.put(domain);
        JsonValue result = execute(model, SEARCH_COUNT.getName(), argms);
    ...
    public Long doCreate(String model, Map<String, Object> vals)
            throws OeRpcException {
        JSONArray argms = new JSONArray();
        System.out.println(new Gson().toJson(vals));
        argms.put(new JSONObject(new Gson().toJson(vals)));
        JsonValue result = execute(model, CREARE.getName(), argms);
        if (!OeUtil.isNULL(result)) {
            return Long.parseLong(result.toString());
        }
        return 0L;
    ...
    public Boolean doWrite(String model, Integer id, Map<String, Object> vals)
            throws OeRpcException {
        JSONArray argms = new JSONArray();
        argms.put(id);
        System.out.println(new JSONObject(new Gson().toJson(vals)));
        argms.put(new JSONObject(new Gson().toJson(vals)));
        JsonValue result = execute(model, WRITE.getName(), argms);
        if (!OeUtil.isNULL(result)) {
    ...
    public JsonValue execute(String model, String method)
            throws OeRpcException {
    ...
    public JsonValue execute(String model, String method, JSONArray args)
            throws OeRpcException {
        return execute(model, method, args, new JSONObject());
    }

    public JsonValue execute(String url, JSONObject jSONObj) throws OeRpcException {
        String reqUrl = this.url + "/" + url;
        return postRequest(client, reqUrl, getCallWith(jSONObj)).get("result");
    }

    public JsonValue execute(String model, String method, JSONArray args,
            JSONObject kwargs) throws OeRpcException {
